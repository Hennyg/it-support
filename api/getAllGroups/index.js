  ...
  try {
    ...
    try {
      groups = await graphGetAll(
        token,
        "/groups?$select=id,displayName,mail,mailNickname,description,groupTypes,mailEnabled,securityEnabled,resourceProvisioningOptions&$top=999"
      );
    } catch {
      groups = await graphGetAll(
        token,
        "/groups?$select=id,displayName,mail,mailNickname,description,groupTypes,mailEnabled,securityEnabled&$top=999"
      );
    }
